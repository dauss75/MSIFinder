...
def get_left_right_base(bed, ref, specbp):
    fastaheck = "/mnt/archive/home/cancer//cancerAGM_vDS/pipeline/fastahack"
    bed = pd.read_table(bed)
    left_base, right_base = [], []
    for r, s, e in zip(bed["chr"], bed["start"], bed["end"]):
        cmdLeft = "%s -r %s:%s-%s %s" % (fastaheck, r, s - specbp, s - 1, ref)
        cmdRight = "%s -r %s:%s-%s %s" % (fastaheck, r, e + 1, e + specbp, ref)
...
def main(args):
    ...
    flank = args.flank
    specbp = args.specbp
    bed2 = get_left_right_base(bed, ref, specbp)
